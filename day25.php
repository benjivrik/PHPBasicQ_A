...
// Day 25 : Challenge X - store and manipulate items
...
// read the second and fourth item added in the table and display
// the information on your webpage inside a html table
...
try {
    ...
    echo "<br><h5><b> Reading records (second and fourth items) 
    ...
    $sql_stmt = $conn->prepare("SELECT id, item_name, item_price, item_category, profile_path 
                                FROM items 
                                WHERE item_name = '".$item2->get_name()."' 
                                OR item_name = '".$item4->get_name()."' ");
    $sql_stmt->execute();
